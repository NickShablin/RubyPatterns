...
def count_special_numbers(n)
  ...
  sum_prime_digits = n.to_s.chars.map(&:to_i).select { |digit| prime?(digit) }.sum
...
puts "Метод 3: Количество специальных чисел для, не являющихся делителями исходного числа, не взаимно простых с ним и взаимно простых с суммой простых цифр числа #{n}: #{count_special_numbers(n)}"